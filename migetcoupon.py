...
def login(name,pwd):
    ...
    time.sleep(1)
    browser.find_element_by_id("username").send_keys(name) #利用帳號標籤的ID，確定位置並輸入
    browser.find_element_by_id("pwd").send_keys(pwd) #利用密碼標籤的ID，確定位置並輸入
...
def main():
    login('name','pwd') #在此輸入name為你的帳號，pwd為你的密碼
    keepclick()
